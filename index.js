...
router.get('/:classes_subjects', (req, res) => {
    const subjects = req.sanitize(req.params.classes_subjects.toString().toUpperCase());
    const classes = data.filter(c => c.subject.toString().toUpperCase() === subjects);
    if (classes.length !==0){
...
router.get('/timetable/:classes_subjects/:course_code/:course_component?', (req, res) => {
    const subjects = data.filter(c => c.subject.toString().toUpperCase() === req.sanitize(req.params.classes_subjects.toString().toUpperCase()));
    const course_code = subjects.filter(c => c.catalog_nbr.toString().toUpperCase() === req.sanitize(req.params.course_code.toString().toUpperCase()));
    ...
    if(course_code.filter(c => c.course_info[0].ssr_component.toUpperCase() === req.sanitize(req.params.course_component))){
...
router.put('/schedule/:name',(req,res)=>{
    ...
    db.get('schedules').push({scheduleName:name,subject:[],courseName:[]}).write();
    res.status(200).send();
});
...
router.get('/disp/schedule', (req,res)=>{
    let scheduleList=[];
    for(let i = 0; i<db.getState().schedules.length; i++){
        scheduleList.push(`Schedule name:${db.getState().schedules[i].scheduleName}, Number of courses:${db.getState().schedules[i].courseName[i].length}`)
    }
